...
# Description: Codigo para actualizacion de archivo de configuración dictionary.py 
#########################################################################################
# Fases del programa.
# 1. Conexion y descarga de la configuracion de ELK
# 2. Construccion de los archivos necesarios de configuraciòn
# 3. Reinicio de los servicios respectivos: multiprocess_monitor_firewall_ssh.py
#########################################################################################
...
class update_dictionary():
    def __init__(self):
        ...
        print("START| {0} update_dictionary | SO {1}".format(datetime.utcnow().isoformat(),self.s_o))
        ...
        if(self.s_o=='Windows'):
            self.fullpath_yml = "dictionary.yml"
        else:
    ...
    def build_yml(self):
        ...
        print("INFO | {0} update_dictionary | Created <heartbeat.yml>".format(datetime.utcnow().isoformat()))
        return 
    ...
    def relaunch_service(self,mode="production"): #mode={"production" , "debug"}
        ...
        #time.sleep(2)
        if(mode=="debug"):
            print("INFO | {0} update_dictionary | Starting heartbeat mode debug.".format(datetime.utcnow().isoformat()))
            #os.system("{0} -c {1} -e".format(self.fullpath_bin, self.fullpath_yml))
        else:#mode="production"
            print("INFO | {0} update_dictionary | Restarting service heartbeat.".format(datetime.utcnow().isoformat()))
            #os.system("{0} restart".format(self.service_bin))
        
if __name__ == "__main__":
    hearbeat = update_dictionary()
